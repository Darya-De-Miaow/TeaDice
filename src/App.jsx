...
// This defines the Tea Sepolia testnet network details needed to connect MetaMask to the correct blockchain.
...
const App = () => {
  ...
  const connectWallet = async () => {
    try {
      ...
      // This uses ethers.js BrowserProvider to connect to MetaMask and get a signer for transactions.
      const provider = new ethers.BrowserProvider(instance);
      const signer = await provider.getSigner();
      // This creates a contract instance using the deployed address and ABI so the frontend can call its functions.
  ...
  const checkNetwork = async () => {
    ...
    try {
      ...
      // This checks if the user is on Tea Sepolia and does a switch to it if not.
      if (network.chainId !== parseInt(TEA_SEPOLIA_PARAMS.chainId, 16)) {
        try {
          ...
          if (switchError.code === 4902) {
            try {
              // This adds Tea Sepolia to MetaMask if the user hasn't got it yet.
              await window.ethereum.request({
  ...
  return (
    <Router future={{ v7_startTransition: true, v7_relativeSplatPath: true }}>
      ...
      <Routes>
        ...
        {/* I updated this to pass account and contract so the NFT marketplace can read the player's live on-chain score */}
        <Route path="/nft-marketplace" element={<NFTMarketplace diceGameContract={diceGameContract} account={account} />} />
